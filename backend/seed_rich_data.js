...
async function seedRichData() {
  console.log('--- Cleaning and Seeding Expanded Dummy Data ---');
  ...
  // 4. Expanded Student List (15 Students across branches, quotas, semesters)
  const studentsList = [
    {
      usn: '1GC22CS001',
      name: 'Rahul Sharma',
  ...
  await AuditLog.create({
    ...
    details: 'System backend initialized with rich multi-student mock dataset.',
  ...
  await AuditLog.create({
    ...
    details: 'Academic Year 2025-26 quota slabs configured.',
    performedBy: 'System Admin',
    timestamp: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString()
  });

  console.log(`✅ Successfully seeded ${studentsList.length} students across CS, EC, IS, ME, CV, AI with active payments & receipts!`);
}

seedRichData().then(() => process.exit(0)).catch(err => {
  console.error(err);
  process.exit(1);
});
